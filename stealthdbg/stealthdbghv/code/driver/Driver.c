    ...
    PUNICODE_STRING RegistryPath)
{
    ...
    UNICODE_STRING DriverName    = RTL_CONSTANT_STRING(L"\\Device\\StealthDbgHypervisorDevice");
    UNICODE_STRING DosDeviceName = RTL_CONSTANT_STRING(L"\\DosDevices\\StealthDbgHypervisorDevice");
    ...
    LogDebugInfo("StealthDbg is loaded :)");
...
DrvUnload(PDRIVER_OBJECT DriverObject)
{
    ...
    RtlInitUnicodeString(&DosDeviceName, L"\\DosDevices\\StealthDbgHypervisorDevice");
    IoDeleteSymbolicLink(&DosDeviceName);
    IoDeleteDevice(DriverObject->DeviceObject);

    DbgPrint("StealthDbg's hypervisor driver unloaded\n");
...
DrvCreate(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    LogDebugInfo("StealthDbg's hypervisor started...");
    //
    ...
    if (VmxInitialize())
    {
        LogDebugInfo("StealthDbg's hypervisor loaded successfully");
        ...
        {
            LogDebugInfo("StealthDbg's debugger loaded successfully");
        ...
        {
            LogError("Err, StealthDbg's debugger was not loaded");
        }
    }
    else
    {
        LogError("Err, StealthDbg's hypervisor was not loaded :(");
    }
