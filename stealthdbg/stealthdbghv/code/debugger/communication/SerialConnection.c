...
SerialConnectionSendEndOfBuffer()
{
    ...
    // Send the end buffer
    //
    KdStealthDbgSendByte(SERIAL_END_OF_BUFFER_CHAR_1, TRUE);
    KdStealthDbgSendByte(SERIAL_END_OF_BUFFER_CHAR_2, TRUE);
    KdStealthDbgSendByte(SERIAL_END_OF_BUFFER_CHAR_3, TRUE);
    KdStealthDbgSendByte(SERIAL_END_OF_BUFFER_CHAR_4, TRUE);
}
                           ...
                           UINT32 * LengthReceived)
{
    ...
    while (TRUE)
    {
        ...
        if (!KdStealthDbgRecvByte(&RecvChar))
        {
            continue;
        }
...
SerialConnectionSend(CHAR * Buffer, UINT32 Length)
{
    ...
    {
        KdStealthDbgSendByte(Buffer[i], TRUE);
    }
...
SerialConnectionSendTwoBuffers(CHAR * Buffer1, UINT32 Length1, CHAR * Buffer2, UINT32 Length2)
{
    ...
    {
        KdStealthDbgSendByte(Buffer1[i], TRUE);
    }
    ...
    {
        KdStealthDbgSendByte(Buffer2[i], TRUE);
    }
                                 ...
                                 UINT32 Length3)
{
    ...
    {
        KdStealthDbgSendByte(Buffer3[i], TRUE);
    }
...
SerialConnectionPrepare(PDEBUGGER_PREPARE_DEBUGGEE DebuggeeRequest)
{
    ...
    //
    KdStealthDbgPrepareDebuggeeConnectionPort(DebuggeeRequest->PortAddress, DebuggeeRequest->Baudrate);
