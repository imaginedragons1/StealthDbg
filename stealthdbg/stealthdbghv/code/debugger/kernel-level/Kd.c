...
/**
 * @brief Sends a StealthDbg response packet to the debugger
 *
 * @param PacketType
 * @param Response
...
/**
 * @brief Sends a StealthDbg logging response packet to the debugger
 *
 * @param OptionalBuffer
 * @param OptionalBufferLength
 * @param OperationCode
...
KdHandleHaltsWhenNmiReceivedFromVmxRoot(UINT32 CurrentCore, PGUEST_REGS GuestRegs)
{
    ...
    //
    // During the debugging of StealthDbg, we realized that whenever an
    ...
    // is not big enough to handle StealthDbg's command dispatching
...
KdDispatchAndPerformCommandsFromDebugger(ULONG CurrentCore, PGUEST_REGS GuestRegs)
{
    ...
    while (TRUE)
    {
        ...
        if (TheActualPacket->Indicator == INDICATOR_OF_StealthDbg_PACKER)
        {
            //
            // Check checksum
            //
        ...
        {
            //
            // It's not a StealthDbg packet, the packet is probably deformed
            //
            LogError("Err, it's not a StealthDbg packet, the packet is probably deformed\n");
            continue;
        }
...
KdHaltSystem(PDEBUGGER_PAUSE_PACKET_RECEIVED PausePacket)
{
    ...
    // of their execution codes and not on StealthDbg routines
    //
    // KdBroadcastHaltOnAllCores();
    //
