...
CommandScriptHelp()
{
    ShowMessages(".script : runs a StealthDbg script.\n\n");
...
CommandScriptRunCommand(std::string Input, vector<string> PathAndArgs)
{
    ...
    CommandExecutionResult = StealthDbgInterpreter(LineContent);
 ...
 */
VOID
StealthDbgScriptReadFileAndExecuteCommand(vector<string> & PathAndArgs)
{
    std::string Line;
    ...
    if (File.is_open())
    {
        ...
        while (std::getline(File, Line))
        {
            ...
            //
            if (StealthDbgCheckMultilineCommand(Line, Reset))
            {
                //
...
CommandScript(vector<string> SplittedCommand, string Command)
{
    ...
    */
    ...
    //
    StealthDbgScriptReadFileAndExecuteCommand(PathAndArgs);
}
