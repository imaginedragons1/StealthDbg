...
CommandListenHelp()
{
    ShowMessages(".listen : listens for a client to connect to StealthDbg (works as "
                 "a guest server).\n\n");

    ShowMessages("note : \tif you don't specify port then StealthDbg uses the "
                 "default port (%s)\n",
                 DEFAULT_PORT);
...
CommandListen(vector<string> SplittedCommand, string Command)
{
    ...
    if (g_IsConnectedToStealthDbgLocally || g_IsConnectedToRemoteDebuggee ||
    ...
    {
        ShowMessages("you're connected to a an instance of StealthDbg, please use "
                     "'.debug close' command\n");
        return;
    }
