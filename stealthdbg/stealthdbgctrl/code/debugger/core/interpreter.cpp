 ...
 * @author anonymous
 * @brief The StealthDbg command interpreter and driver connector
 * @details
 * @version 0.1
 * @date 2020-04-11
 *
 ...
 */
int
StealthDbgInterpreter(char * Command)
{
    ...
    // still some of the commands should be handled in the local StealthDbg
    //
...
/**
 * @brief Show signature of StealthDbg
 *
 * @return VOID
 */
VOID
StealthDbgShowSignature()
{
    if (g_IsConnectedToRemoteDebuggee)
    {
        ...
        //
        ShowMessages("[%s:%s] StealthDbg> ", g_ServerIp.c_str(), g_ServerPort.c_str());
    }
    else if (g_ActiveProcessDebuggingState.IsActive)
    {
        ...
        //
        ShowMessages("%x:%x u%sStealthDbg> ",
    ...
    else if (g_IsSerialConnectedToRemoteDebuggee)
    {
        ...
        //
        ShowMessages("%x: kStealthDbg> ", g_CurrentRemoteCore);
    }
    else
    {
        //
 ...
 */
bool
StealthDbgCheckMultilineCommand(std::string & CurrentCommand, bool Reset)
{
    if (Reset)
    {
