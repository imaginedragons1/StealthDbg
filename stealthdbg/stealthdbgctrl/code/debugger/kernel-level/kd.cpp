...
extern BOOLEAN                              g_IsConnectedToStealthDbgLocally;
...
KdSendStepPacketToDebuggee(DEBUGGER_REMOTE_STEPPING_REQUEST StepRequestType)
{
    ...
    if (StepRequestType == DEBUGGER_REMOTE_STEPPING_REQUEST_STEP_OVER)
    {
        ...
        //
        if (StealthDbgCheckWhetherTheCurrentInstructionIsCall(
...
/**
 * @brief Sends a StealthDbg packet to the debuggee
 *
    ...
    DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION RequestedAction)
{
    ...
    //
    Packet.Indicator       = INDICATOR_OF_StealthDbg_PACKER;
...
/**
 * @brief Sends a StealthDbg packet + a buffer to the debuggee
 *
 * @param RequestedAction
 * @param Buffer
 * @param BufferLength
...
KdPrepareAndConnectDebugPort(const char * PortName, DWORD Baudrate, UINT32 Port, BOOLEAN IsPreparing, BOOLEAN IsNamedPipe)
{
    ...
    */
    ...
    if (IsPreparing)
    {
        ...
        //
        g_IsConnectedToStealthDbgLocally = TRUE;
        ...
        //
        // StealthDbgInterpreter("!syscall script { print(@rax); }");
        // StealthDbgInterpreter("!epthook fffff801`639b1030 script { print(@rax);
        // }"); StealthDbgInterpreter("!msrwrite script { print(@rax); }");
        //
...
KdReloadSymbolsInDebuggee(BOOLEAN PauseDebuggee, UINT32 UserProcessId)
{
    ...
    // we choose the StealthDbg's process to load its modules
    //
...
KdCloseConnection()
{
    ...
    if (g_IsSerialConnectedToRemoteDebugger)
    {
        if (g_IsConnectedToStealthDbgLocally && g_IsDebuggerModulesLoaded)
        {
            StealthDbgUnload();
        }
    }
